...
class BizAccount:
    def bizaccount_create_vorwands(self, cookies, city_code, rubrics=None, xml_override=None):
        phone = prepare_phone()
        ...
        # Если пользователь прислал вручную XML — используем его
        if xml_override:
            xml = xml_override
        else:
    ...
    <BizaccountVorwandRequestExtended Type="Create" CityCode="{city_code}">
        ...
        try:
            ...
            # 🧩 Оборачиваем результат, если это просто число или строка (например, vorwand_id)
            if isinstance(parsed, (int, str)):
                result = {"vorwand_id": parsed}
            elif isinstance(parsed, dict):
                result = parsed
            else:
                result = {"raw_response": str(parsed)}

            # ✅ Добавляем исходный XML в ответ
            result["xml_request"] = xml
